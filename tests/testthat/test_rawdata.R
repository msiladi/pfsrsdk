...
test_that(paste("test get expt., expt. sample, raw and intermediate data", instance), {
  ...
  exptSamples <- CoreAPIV2::getExperimentSamples(con$coreApi, "BITTERNESS EXPERIMENT",
    barcode = "BTXP2",
    useVerbose = verbose
  )
  ...
  # get expt. container
  exptContainer <- CoreAPIV2::getExperimentContainers(con$coreApi, "BITTERNESS EXPERIMENT",
    barcode = "BTXP2",
    useVerbose = verbose
  )
  ...
  rdUpdate2 <-
    updateExperimentSampleRawData(
      ...
      1,
      values = list(
        DATA_VALUE = as.numeric(rawData$entity[rawData$entity$CI_CELL == 1, ]$DATA_VALUE),
  ...
  expect_equal(rdUpdate2$entity$DATA_VALUE,
    as.numeric(rawData$entity[rawData$entity$CI_CELL == 1, ]$DATA_VALUE),
    all = verbose
  )
  expect_equal(rdUpdate2$entity$CI_ACCEPT,
    as.logical(rawData$entity[rawData$entity$CI_CELL == 1, ]$CI_ACCEPT),
    all = verbose
  )
  ...
  intermediateData <- CoreAPIV2::getExperimentSamplesIntermediateData(con$coreApi,
  ...
  ad <- CoreAPIV2::getExperimentSamplesAssayData(
    coreApi = con$coreApi, assayType = "BITTERNESS_ASSAY",
