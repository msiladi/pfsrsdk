...
test_that(paste("test updateExperimentSampleData() on: ", env$auth), {
  ...
  result <- CoreAPIV2::updateExperimentSampleData(con$coreApi, data$experimentAssayType, data$experimentSampleBarcode, jsonlite::unbox(data$experimentAssayUpdateAttrList), useVerbose = verbose)
