...
test_that("full sample to container cycle", {
  ...
  return <- CoreAPIV2::createEntity(
    coreApi = con$coreApi, entityType = "PATIENT_SAMPLE",
  ...
  sl <- CoreAPIV2::createSampleLot(con$coreApi,
    sampleType = "PATIENT_SAMPLE", sampleBarcode = b$Barcode, body = NULL,
    useVerbose = verbose
  )
  ...
  setCont <- CoreAPIV2::setWellContents(
    coreApi = con$coreApi, containerType = "VIAL",
  ...
  contWell <- CoreAPIV2::getWellContents(con$coreApi,
    containerType = "VIAL", cont_barcode, "1",
    useVerbose = verbose
  )
  sourceCellID <- contWell$entity$Id

  print(sourceCellID)
  cont2Well <- CoreAPIV2::getWellContents(con$coreApi,
    containerType = "VIAL", cont2_barcode, "1",
    useVerbose = verbose
  )
