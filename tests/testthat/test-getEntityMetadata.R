...
lapply(environments, function(x) {
  ...
  test_that(paste("test getEntityMetadata() on:", x), {
    out <- CoreAPIV2::getEntityMetadata(con$coreApi, TESTPOCOTYPE, useVerbose = verbose)
  ...
  test_that(paste("test getEntityMetadata() errors out if entityType does not exist on:", x), {
    expect_error(CoreAPIV2::getEntityMetadata(con$coreApi, TESTPOCONONEXISTENTTYPE, useVerbose = verbose))
  })
