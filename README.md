...
`devtools::build(path = "build")`|`./gradlew buildPackage`
`devtools::install(pkg = "build")`|`.gradlew installPackage`
...
we are also able to generate Docker images of the installed package and of the
`pkgdown` documentation site. These two images are built on top of the 
...
```
./gradlew buildROdataSdkPackageImage
